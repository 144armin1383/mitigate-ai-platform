Mission: Self-Healing Phase 3B-1 Audit Integration Adapter v2
...
The previous generation failed because the test source embedded a repository-
forbidden private-key marker literally.

Do NOT include complete repository-forbidden fragments anywhere in either
...
For private-key-like redaction tests, construct synthetic sensitive input at
runtime from harmless pieces.
...
Then compose the synthetic marker dynamically at runtime.

The complete forbidden marker must not occur literally in generated source.

Tests must verify:

- the sensitive synthetic body/value is absent after sanitization
- [REDACTED] is present
- audit output contains no raw private-key-like material

Do not require the original sensitive marker itself to remain visible.

Apply the same source-safety principle to every Mission Runner forbidden
fragment.

All original Phase 3B-1 requirements remain mandatory.
