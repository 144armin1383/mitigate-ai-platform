(() => {
  ...
  async function decide(button, decision) {
    ...
    const message = decision === 'approve'
      ? `Approve reviewed mission ${missionId} and fast-forward its verified branch into main?`
      : `Reject mission ${missionId}? It will be removed from the active approval queue but its decision audit record will be preserved.`;
    ...
    try {
      await api(`/missions/${encodeURIComponent(missionId)}/${decision}`, {method: 'POST', body: '{}'});
      if (resultEl) {
        resultEl.className = 'mitigate-result mitigate-success';
        resultEl.textContent = decision === 'approve'
          ? 'Approved and merged successfully. Decision recorded.'
          : 'Rejected and removed from active approvals. Decision recorded.';
      }
      await load();
    } catch (error) {
      if (resultEl) {
        resultEl.className = 'mitigate-result mitigate-error';
        resultEl.textContent = `${decision === 'approve' ? 'Approval' : 'Rejection'} blocked: ${error.message}`;
      }
